...
function PricingFallback({ showBackToApp }: { showBackToApp: boolean }) {
    return (
...
export default function Product() {
    ...
    const showSubscriptionView = router.query.view === 'subscription';

    const isSubscribed = subscription?.status === 'active';
    const showApp = isSubscribed && !showSubscriptionView;

    return (
        <main className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800">
            {/* Top bar: Subscription link when in app, User menu */}
            <div className="absolute top-4 right-4 flex items-center gap-4">
                {showApp && (
                    <Link
                        href="/product?view=subscription"
            ...
            {/* Show consultation app when subscribed (and not viewing subscription); otherwise show pricing */}
            {isLoading ? (
                ...
                <PricingFallback showBackToApp={isSubscribed} />
            )}
        </main>
    );
}